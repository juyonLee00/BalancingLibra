...
namespace JusticeScale.Scripts.Scales
{
    public class OverlapScale : Scale
    {
        ...
        [Min(0)] [SerializeField]
        private float capsuleOffsetHeight = 0f; // The vertical offset from the scale’s position to the start of the capsule
        ...
        private float CalculateWeightInCapsule()
        {
            ...
            foreach (var collider in colliders)
            {
                ...
                if (rb != null && !_detectedObjects.Contains(rb))
                {
                    ...
                    //감지된 물체가 Ball이면 현재 스케일 등록.
                    BallCollisionSensor sensor = rb.GetComponent<BallCollisionSensor>();
                    if(sensor != null)
                    {
                        sensor.RegisterScale(this);
                    }
                }
            }
